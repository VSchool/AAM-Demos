/* TodayList — the live Today tab. A FlatList of channels (v2) with the
   ...
   v3: each row is now a ThrowRow — swipe (or tap) the switch to complete a
   habit. The store's toggleDone commits the throw: the LED snaps green and
   the streak ticks up.
...
export default function TodayList() {
  const { habits, addHabit, toggleDone, progress } = useHabitStore();
    ...
    <View style={{ flex: 1 }}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          // v3's tour rings the first channel to point at the swipe.
