...
interface HabitStoreValue {
  ...
  /** throw the switch — complete (done, streak +1) or un-throw (today, streak -1). */
...
export function HabitStoreProvider({ children }: { children: ReactNode }) {
  ...
  // v3's "Throw" commits here. Completing a channel bumps the streak (and the
  // best-streak if it's a new high); un-throwing it walks the streak back.
  // A rest channel that gets thrown counts as done — Pulse never punishes.
  ...
  const value = useMemo<HabitStoreValue>(
    () => ({ habits, addHabit, toggleDone, progress }),
    [habits, addHabit, toggleDone, progress],
  );
