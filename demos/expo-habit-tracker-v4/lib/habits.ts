...
/* ---- Streaks (meter-bridge) derivations — read by v1's Streaks tab ---- */

/** The last `n` days of a habit's history (newest last), for the LED bars. */
...
/**
 * Whole-roster 30-day uptime: the share of all logged days that were a
 * completion OR an intentional rest. Rest days count as kept — Pulse never
 * punishes a planned pause — so only true misses pull the gauge down.
 */
export function uptimePct(): number {
  let kept = 0;
  let total = 0;
  for (const h of HABITS) {
    for (const cell of h.history) {
      total += 1;
      if (cell !== "miss") kept += 1;
    }
  }
  return total === 0 ? 0 : Math.round((kept / total) * 100);
}
